# Runtime Tcl commands to interact with - read_spi_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/read_spi_v1_0_include.tcl
...
# Test: Read_from_SPI
set wdata_read_from_spi [get_init_data 0]
create_hw_axi_txn w_read_from_spi_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_read_from_spi}
create_hw_axi_txn r_read_from_spi_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_read_from_spi_addr
run_hw_axi w_read_from_spi_addr
run_hw_axi r_read_from_spi_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_read_from_spi_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - Read_from_SPI"
} else {
	puts "Data comparison test fail for - Read_from_SPI, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}

# Test: Write_to_RAM
set wdata_write_to_ram [get_init_data 1]
create_hw_axi_txn w_write_to_ram_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_write_to_ram}
create_hw_axi_txn r_write_to_ram_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_write_to_ram_addr
run_hw_axi w_write_to_ram_addr
run_hw_axi r_write_to_ram_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_write_to_ram_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 1 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - Write_to_RAM"
} else {
	puts "Data comparison test fail for - Write_to_RAM, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
